...
export default function ReportPage() {
  ...
  const handleVerify = async() => {
    ...
    try {
      ...
      const systemPrompt = `You are an expert in waste management and recycling. Analyze this image and provide:
        1. The type of waste (e.g., plastic, paper, glass, metal, organic)
        2. An estimate of the quantity or amount (in kg or liters)
        3. Your confidence level in this assessment (as a percentage)
        
        Respond in JSON object like this:
        {
        ...
        // const sanitizedText = DOMPurify.sanitize(result.response.text());
        ...
        try {
          ...
          console.error(`Failed to parse JSON res`, error);
          setVerificationStatus('failure');

        }

    } catch (error) {
      console.error(`Error in verifying garbage type`, error);
      setVerificationStatus('failure');
    }
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    if(verificationStatus !== 'success' || !user) {
      toast.error(`Please verify the garbage before submitting or log in`);
      return;
    }
